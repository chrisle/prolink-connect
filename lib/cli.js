...
async function cli() {
    ...
    network.deviceManager.on('connected', d => {
        signale_1.default.star('New device: %s [id: %s]', d.name, d.id);
    });
    ...
    network.statusEmitter.on('status', async (state) => {
        ...
        if (buf) {
            fs_1.default.writeFileSync(track.fileName, buf, 'binary');
 ...
 */
exports.STARTUP_STAGE_INTERVAL = 300;
// prettier-ignore
/**
 ...
 */
exports.VIRTUAL_CDJ_NAME = 'ProLink-Connect';
/**
...
const GetArtworkThumbnail = __importStar(__webpack_require__(/*! ./getArtworkThumbnail */ "./src/db/getArtworkThumbnail.ts"));
...
class Database {
    ...
    async getFile(opts) {
        ...
        let artwork = null;
        if (strategy === LookupStrategy.Remote) {
            artwork = await GetFile.viaRemote(__classPrivateFieldGet(this, _Database_remoteDatabase, "f"), device, callOpts);
        }
        if (strategy === LookupStrategy.Local) {
            artwork = await GetFile.viaLocal(__classPrivateFieldGet(this, _Database_localDatabase, "f"), device, callOpts);
        }
        if (strategy === LookupStrategy.NoneAvailable) {
            tx.setStatus(telemetry_1.SpanStatus.Unavailable);
        }
     ...
     */
    async getArtworkThumbnail(opts) {
        const { deviceId, trackType, trackSlot, span } = opts;
        const tx = span
        ...
        let artwork = null;
        if (strategy === LookupStrategy.Remote) {
            artwork = await GetArtworkThumbnail.viaRemote(__classPrivateFieldGet(this, _Database_remoteDatabase, "f"), callOpts);
        }
        if (strategy === LookupStrategy.Local) {
            artwork = await GetArtworkThumbnail.viaLocal(__classPrivateFieldGet(this, _Database_localDatabase, "f"), device, callOpts);
        }
        if (strategy === LookupStrategy.NoneAvailable) {
            tx.setStatus(telemetry_1.SpanStatus.Unavailable);
        }
...
const defaultConfig = {
    deviceTimeout: 10000,
    vcdjName: constants_1.VIRTUAL_CDJ_NAME,
};
/**
...
class DeviceManager {
    constructor(announceSocket, config) {
        ...
        _DeviceManager_handleAnnounce.set(this, (message) => {
            ...
            if (device.name === __classPrivateFieldGet(this, _DeviceManager_config, "f").vcdjName) {
                return;
            }
...
const position_1 = __importDefault(__webpack_require__(/*! src/status/position */ "./src/status/position.ts"));
const types_1 = __webpack_require__(/*! src/types */ "./src/types.ts");
const utils_1 = __webpack_require__(/*! src/utils */ "./src/utils/index.ts");
const Telemetry = __importStar(__webpack_require__(/*! src/utils/telemetry */ "./src/utils/telemetry.ts"));
const telemetry_1 = __webpack_require__(/*! src/utils/telemetry */ "./src/utils/telemetry.ts");
...
async function bringOnline(config) {
    Telemetry.setTag('connectionId', (0, crypto_1.randomUUID)());
    const tx = Telemetry.startTransaction({ name: 'bringOnline' });
    // Socket used to listen for devices on the network
    const announceSocket = dgram_1.default.createSocket({ type: 'udp4', reuseAddr: true });
    // Socket used to listen for beat timing information
    const beatSocket = dgram_1.default.createSocket({ type: 'udp4', reuseAddr: true });
    // Socket used to listen for status packets
    const statusSocket = dgram_1.default.createSocket({ type: 'udp4', reuseAddr: true });
    try {
...
class ProlinkNetwork {
    ...
    constructor({ config, announceSocket, beatSocket, statusSocket, deviceManager, statusEmitter, positionEmitter, }) {
        ...
        __classPrivateFieldSet(this, _ProlinkNetwork_positionEmitter, positionEmitter, "f");
    ...
    async autoconfigFromPeers() {
        ...
        if (iface === null) {
            tx.setStatus(telemetry_1.SpanStatus.InternalError);
    ...
    connect() {
        ...
        const vcdj = (0, virtualcdj_1.getVirtualCDJ)(__classPrivateFieldGet(this, _ProlinkNetwork_config, "f").iface, __classPrivateFieldGet(this, _ProlinkNetwork_config, "f").vcdjId, __classPrivateFieldGet(this, _ProlinkNetwork_config, "f").vcdjName);
        // Update device manager to filter out our VCDJ name
        if (__classPrivateFieldGet(this, _ProlinkNetwork_config, "f").vcdjName !== undefined) {
     ...
     */
    get positionEmitter() {
        return __classPrivateFieldGet(this, _ProlinkNetwork_state, "f") === types_1.NetworkState.Connected ? __classPrivateFieldGet(this, _ProlinkNetwork_positionEmitter, "f") : null;
    }
    /**
 ...
 */
async function fetchFile({ device, slot, path, onProgress, span, chunkSize, }) {
    const tx = span
 ...
 * file into memory.
 */
async function fetchFile(conn, file, onProgress, span, readSize) {
 ...
 */
function slicePadding(io, padding) {
    if (padding > 0) {
        io.slice(padding);
    }
}
...
class Message {
    ...
    static async fromStream(stream, expect, span) {
        ...
        if (messageType.value !== expect) {
            ...
            tx.setStatus(telemetry_1.SpanStatus.FailedPrecondition);
            tx.finish();
...
const fieldsToItem = (args) => {
    ...
    if (transformer === undefined) {
        transformer = () => null;
        Telemetry.captureMessage(`No item transformer registered for item type ${type}`, Telemetry.Severity.Error);
    }
...
class StatusEmitter {
    ...
    constructor(statusSocket) {
        ...
        _StatusEmitter_handleStatus.set(this, (message) => {
            ...
            const onAir = (0, utils_1.onAirFromPacket)(message);
            if (onAir !== undefined) {
                return __classPrivateFieldGet(this, _StatusEmitter_emitter, "f").emit('onAir', onAir);
            }
            return undefined;
        });
 ...
 */
function positionFromPacket(packet) {
    if (packet.indexOf(constants_1.PROLINK_HEADER) !== 0) {
        return undefined;
    }
 ...
 */
function onAirFromPacket(packet) {
    if (packet.indexOf(constants_1.PROLINK_HEADER) !== 0) {
        return undefined;
    }
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.Severity = exports.SpanStatus = exports.isTelemetryEnabled = exports.getSentryDsn = void 0;
exports.init = init;
 ...
 */
const getSentryDsn = () => process.env.PROLINK_CONNECT_SENTRY_DSN || process.env.SENTRY_DSN;
...
const isTelemetryEnabled = () => {
    ...
    return value === 'true' || value === '1' || !!(0, exports.getSentryDsn)();
};
    ...
    return {
        startChild(context) {
            ...
            const child = Sentry.startInactiveSpan({
                name: (_b = (_a = context === null || context === void 0 ? void 0 : context.name) !== null && _a !== void 0 ? _a : context === null || context === void 0 ? void 0 : context.description) !== null && _b !== void 0 ? _b : 'child',
                op: context === null || context === void 0 ? void 0 : context.op,
                attributes: context === null || context === void 0 ? void 0 : context.data,
            });
...
function init(options) {
    var _a;
    if (!(0, exports.isTelemetryEnabled)()) {
        return;
    }
    const dsn = (_a = options === null || options === void 0 ? void 0 : options.dsn) !== null && _a !== void 0 ? _a : (0, exports.getSentryDsn)();
    if (!dsn) {
        return;
    }
    Sentry.init({
...
function startTransaction(context) {
    ...
    if ((0, exports.isTelemetryEnabled)()) {
        const span = Sentry.startInactiveSpan({
            name: (_b = (_a = context.name) !== null && _a !== void 0 ? _a : context.description) !== null && _b !== void 0 ? _b : 'transaction',
            op: context.op,
 ...
 */
const getVirtualCDJ = (iface, id, name = constants_1.VIRTUAL_CDJ_NAME) => ({
    id,
    name,
...
(function (StartupStage) {
    ...
    StartupStage[StartupStage["InitialAnnounce"] = 10] = "InitialAnnounce";
    /** First-stage device number claim (0x00) */
    StartupStage[StartupStage["FirstStageClaim"] = 0] = "FirstStageClaim";
    /** Second-stage device number claim (0x02) */
    StartupStage[StartupStage["SecondStageClaim"] = 2] = "SecondStageClaim";
    /** Final-stage device number claim (0x04) */
    StartupStage[StartupStage["FinalStageClaim"] = 4] = "FinalStageClaim";
    /** Keep-alive packets (0x06) */
    StartupStage[StartupStage["KeepAlive"] = 6] = "KeepAlive";
})(StartupStage || (StartupStage = {}));
/**
 * Build stage 0x0a packet: Initial announcement (CDJ-3000 compatible).
 * Sent 3 times at 300ms intervals.
 */
function makeStage0aPacket(device) {
    const isHighPlayerNumber = device.id >= 5;
    const parts = [
        ...constants_1.PROLINK_HEADER,
        ...[0x0a, 0x00],
        ...(0, utils_1.buildName)(device),
        ...[0x01],
 ...
 */
function makeStage00Packet(device, counter) {
    const isHighPlayerNumber = device.id >= 5;
    const parts = [
        ...constants_1.PROLINK_HEADER,
        ...[0x00, 0x00],
 ...
 * Sent 3 times at 300ms intervals with counter N (1, 2, 3).
 */
function makeStage02Packet(device, counter) {
    const isHighPlayerNumber = device.id >= 5;
    const parts = [
        ...constants_1.PROLINK_HEADER,
        ...[0x02, 0x00],
 ...
 */
function makeStage04Packet(device, counter) {
    const isHighPlayerNumber = device.id >= 5;
    const parts = [
        ...constants_1.PROLINK_HEADER,
        ...[0x04, 0x00],
 ...
 */
function makeStage06Packet(device, peerCount) {
    const isHighPlayerNumber = device.id >= 5;
    const parts = [
        ...constants_1.PROLINK_HEADER,
        ...[0x06, 0x00],
...
class Announcer {
    constructor(vcdj, announceSocket, deviceManager, iface, fullStartup = false) {
         ...
         */
        _Announcer_currentStage.set(this, StartupStage.InitialAnnounce);
        /**
         * Packet counter within current stage (1, 2, 3)
         */
        _Announcer_stageCounter.set(this, 0);
        /**
         * Listener for incoming conflict packets
         */
        _Announcer_conflictListener.set(this, void 0);
        /**
...
_Announcer_announceSocket = new WeakMap(), _Announcer_deviceManager = new WeakMap(), _Announcer_vcdj = new WeakMap(), _Announcer_intervalHandle = new WeakMap(), _Announcer_fullStartup = new WeakMap(), _Announcer_currentStage = new WeakMap(), _Announcer_stageCounter = new WeakMap(), _Announcer_conflictListener = new WeakMap(), _Announcer_iface = new WeakMap(), _Announcer_instances = new WeakSet(), _Announcer_startFullStartup = function _Announcer_startFullStartup() {
    ...
    if (__classPrivateFieldGet(this, _Announcer_intervalHandle, "f")) {
        clearTimeout(__classPrivateFieldGet(this, _Announcer_intervalHandle, "f"));
        __classPrivateFieldSet(this, _Announcer_intervalHandle, undefined, "f");
    }
    // Find an available device ID
    const usedIds = new Set([...__classPrivateFieldGet(this, _Announcer_deviceManager, "f").devices.values()].map(d => d.id));
    let newId = null;
    ...
    switch (__classPrivateFieldGet(this, _Announcer_currentStage, "f")) {
        case StartupStage.InitialAnnounce:
            packet = makeStage0aPacket(__classPrivateFieldGet(this, _Announcer_vcdj, "f"));
            break;
        case StartupStage.FirstStageClaim:
            packet = makeStage00Packet(__classPrivateFieldGet(this, _Announcer_vcdj, "f"), __classPrivateFieldGet(this, _Announcer_stageCounter, "f"));
            break;
        case StartupStage.SecondStageClaim:
            packet = makeStage02Packet(__classPrivateFieldGet(this, _Announcer_vcdj, "f"), __classPrivateFieldGet(this, _Announcer_stageCounter, "f"));
            break;
        case StartupStage.FinalStageClaim:
            packet = makeStage04Packet(__classPrivateFieldGet(this, _Announcer_vcdj, "f"), __classPrivateFieldGet(this, _Announcer_stageCounter, "f"));
            break;
    ...
    if (__classPrivateFieldGet(this, _Announcer_stageCounter, "f") >= 3) {
        // Move to next stage
        __classPrivateFieldGet(this, _Announcer_instances, "m", _Announcer_advanceStage).call(this);
    }
    // Schedule next packet (300ms for startup stages)
    __classPrivateFieldSet(this, _Announcer_intervalHandle, setTimeout(() => __classPrivateFieldGet(this, _Announcer_instances, "m", _Announcer_sendStagePackets).call(this), constants_1.STARTUP_STAGE_INTERVAL), "f");
}, _Announcer_advanceStage = function _Announcer_advanceStage() {
    __classPrivateFieldSet(this, _Announcer_stageCounter, 0, "f");
    switch (__classPrivateFieldGet(this, _Announcer_currentStage, "f")) {
        case StartupStage.InitialAnnounce:
            __classPrivateFieldSet(this, _Announcer_currentStage, StartupStage.FirstStageClaim, "f");
            break;
        case StartupStage.FirstStageClaim:
            __classPrivateFieldSet(this, _Announcer_currentStage, StartupStage.SecondStageClaim, "f");
            break;
        case StartupStage.SecondStageClaim:
            __classPrivateFieldSet(this, _Announcer_currentStage, StartupStage.FinalStageClaim, "f");
            break;
        case StartupStage.FinalStageClaim:
            __classPrivateFieldSet(this, _Announcer_currentStage, StartupStage.KeepAlive, "f");
            break;
    }
