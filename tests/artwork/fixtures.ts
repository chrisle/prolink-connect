...
export function createMp3WithArtwork(imageData: Buffer, pictureType: number = 3): Buffer {
...
export function createFlacWithArtwork(imageData: Buffer, pictureType: number = 3): Buffer {
  const mimeType = Buffer.from('image/jpeg', 'utf8');

  const pictureData = Buffer.alloc(32 + mimeType.length + imageData.length);
  let offset = 0;

  pictureData.writeUInt32BE(pictureType, offset); offset += 4;
  pictureData.writeUInt32BE(mimeType.length, offset); offset += 4;
  mimeType.copy(pictureData, offset); offset += mimeType.length;
  pictureData.writeUInt32BE(0, offset); offset += 4;
  pictureData.writeUInt32BE(1, offset); offset += 4;
  pictureData.writeUInt32BE(1, offset); offset += 4;
  pictureData.writeUInt32BE(24, offset); offset += 4;
  pictureData.writeUInt32BE(0, offset); offset += 4;
  pictureData.writeUInt32BE(imageData.length, offset); offset += 4;
